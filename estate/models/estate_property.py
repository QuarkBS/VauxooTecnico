...
class EstateProperty(models.Model):
    ...
    date_availability = fields.Date(
        copy=False, default=lambda self: fields.Datetime.add(fields.Datetime.today(), month=3)
    )
    ...
    _sql_constraints = [
        ...
        ("check_selling_price", "CHECK(selling_price >= 0)", "Selling price must always be a positive value or zero"),
    ]
    ...
    def _compute_best_price(self):
        for record in self:
            maps = record.offer_ids.mapped("price")
            if len(maps) > 0:
                record.best_price = max(maps)
            else:
                record.best_price = 0
    ...
    def _assing_selling_price(self):
        for record in self:
            if (
                tools.float_utils.float_compare(record.selling_price, 0.9 * record.expected_price, precision_digits=2)
                < 0
            ):
    ...
    def checkdelete(self):
        for record in self:
            if record.state not in ("new" or "cancelled"):
